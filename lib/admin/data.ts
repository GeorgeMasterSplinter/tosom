/**
 * ToSom — Admin Data Layer
 * 
 * Hentar data for admin-panel: brukarar, profiler, matcher, samtalar, innsikter.
 */

import { prisma } from '@/lib/prisma';
import { MatchStatus, ResonanceLevel, JourneyPhase } from '@prisma/client';
...
export async function getAdminStats() {
  const [userCount, matchCount, conversationCount, messageCount, insightCount, activeMatchCount] = await Promise.all([
    ...
    prisma.matchInsight.count(),
    prisma.match.count({
      where: { status: 'active' as MatchStatus },
    }),
  ]);
  ...
  return {
    users: { total: userCount, last7d: newUsersLast7d, last30d: newUsersLast30d },
...
export async function getAllUsers(page = 1, limit = 20) {
  ...
  counts.forEach(c => {
    matchCounts.set(c.userAId, (matchCounts.get(c.userAId) || 0) + 1);
  });
  ...
  return users.map(u => ({
    ...
    profile: u.profile
      ? { identityName: u.profile.identityName, age: u.profile.age }
      : undefined,
  ...
  return {
    ...
    profile: user.profile
      ? {
          ...
          maturityLevel: user.profile.maturityLevel,
          relationshipStyle: user.profile.relationshipStyle,
          deepProfileStep: user.profile.deepProfileStep,
        }
      : null,
    journey: user.journey
      ? { phase: user.journey.phase, day: user.journey.day, completedDays: user.journey.completedDays }
      : null,
    matchCount: 0,
    conversationCount: 0,
  };
...
export async function getAllProfiles(page = 1, limit = 20) {
  ...
  return profiles.map(p => ({
    ...
    deepProfileStep: p.deepProfileStep,
    deepProfileComplete: p.deepProfileStep === 'SUMMARY',
...
export async function getProfileById(id: string): Promise<AdminProfile | null> {
  ...
  const deepProfileComplete = profile.deepProfileStep === 'SUMMARY';
  ...
  return {
    ...
    deepProfileStep: profile.deepProfileStep,
    deepProfileComplete,
...
export async function getConversationById(id: string): Promise<AdminConversationDetail | null> {
  const conversation = await prisma.conversation.findUnique({
    ...
    include: {
      messages: {
        ...
        include: {
          sender: { select: { profile: { select: { identityName: true } } } },
        },
      },
      userA: { select: { profile: { select: { identityName: true } } } },
      userB: { select: { profile: { select: { identityName: true } } } },
  ...
  return {
    ...
    messages: conversation.messages.map(m => ({
      id: m.id,
      senderId: m.senderId,
      senderName: m.sender.profile?.identityName ?? 'Ukjent',
      content: m.content,
      type: m.type,
      createdAt: m.createdAt.toISOString(),
    })),
...
export async function getInsightById(id: string): Promise<AdminInsightDetail | null> {
  const insight = await prisma.matchInsight.findUnique({
    where: { id },
    include: {
      match: {
        include: {
  ...
  return {
    ...
    model: insight.model,
    tokensOut: insight.tokensOut,
    createdAt: insight.createdAt.toISOString(),
    match: {
      id: insight.match.id,
      userAId: insight.match.userAId,
      userBId: insight.match.userBId,
      userAName: insight.match.userA.profile?.identityName ?? 'Ukjent',
      userBName: insight.match.userB.profile?.identityName ?? 'Ukjent',
      score: insight.match.score,
      resonanceLevel: insight.match.resonanceLevel,
      status: insight.match.status,
      createdAt: insight.match.createdAt.toISOString(),
    },
  };
}