# TOSOM — BETA-DRIFTSPLAN v1.1

**Dato:** 2026-08-24
**Commit:** `0a0e47c`
**Status:** Aktiv. Gjelder fra siste kodeoppgave til beta er avsluttet.
**Kanonisk kilde:** `TOSOM-SUPER-MASTERPLAN-v2.0.md`
**Følgedokumenter:** `BETA-TEST-v1.0.md` (tilgangsmodell), `ACT-PIPELINE-v1.0.md`, `ACT-STATE.json`
...
Dette dokumentet beskriver ikke hva Tosom er — det står i SUPER-MASTERPLAN. Det beskriver **hva som gjenstår før første tester, og hvordan plattformen driftes når brukerne er inne.**
...
### 🟢 IMPLEMENTERT
Kilde: `docs/ACT-STATE.json` (oppdatert 2026-08-24).

| Mål | Verdi |
|---|---|
| Tester | 311/311 grønne (35 suiter) |
| Typesjekk | 0 feil |
| Produksjonsbuild | Verifisert |
| Fase | `apen-beta-test` |

Alle fire runder i SUPER-MASTERPLAN §15 er gjennomført: blokkere, PDF-eksport, beta-drift og opprydding. 2026-08-24 fikk motoren to nye dealbreakere (kjønn og alder, WP1), og onboarding fikk isolert draft-lagring med prefill (WP2).
...
| ID | Sak | Status |
|---|---|---|
| B-1 | Magic link sendes aldri | ✅ lukket — erstattet av e-post + passord (auto-registrering) |
| B-2 | Vipps-callback er død kode | ✅ lukket — CTA-er peker til `/login`, Vipps bak feature-flag |
| B-3 | PDF-eksport mangler før sletting | ✅ lukket — `app/api/journey/export-pdf/route.ts` |
| B-4 | Admin-endepunkter kan eskaleres | ✅ lukket — `adminAuthGuard()` |
| B-5 | E-post-rørlegging | ✅ lukket — `lib/email/` mot Resend |
| B-6 | Matching ventet på lørdag | ✅ lukket — manuell matching via `/admin/tools` |
| B-7 | Cookiesalt kastet brukere ut | ✅ lukket — `middleware.ts` leste rett cookie/salt |
...
| F-8 | Beta-sjekklisten | George, manuelt | Ti manuelle punkter i `BETA-TEST` §10: Vercel-secrets, `MAINTENANCE_MODE` av, `DEV_LOGIN` av, admin-passord, support@/noreplay@-post. |

Tilgangsmodellen er **åpen** (`BETA-TEST` §3): ingen invitasjonsport, ingen koder. George sier fra til de han ønsker; testerne lager konto selv på tosom.no.
...
**Et panel som viser oppdiktede tall er verre enn ingen panel** — derfor var D-2 og D-3 forutsetningen for første tester. De er nå mot sannt data.
...
**Hvis kø-størrelsen er rød (0):** ingen står i kø. Under beta er dette forventet i starten — det betyr at flere tester må inn, ikke at noe er ødelagt.
...
Se også: hvor mange registreringer er fullført, hvor mange reiser er i hver fase, og om noen par har stått stille en hel uke.
...
| Mål | Kriterium |
|---|---|
| Fullført onboarding | ≥ 60 % av kontaktet testere som logger inn |
...
DI-1 er en tillitsregel. Panelet skal vise **at** en samtale finnes og hvor aktiv den er — ikke hva som ble sagt. DI-4 er grunnen til at D-2 og D-3 må lukkes før første tester.
...
| 5 | BETA-TEST §10 — sjekklisten før første tester | BETA-TEST | ⏳ manuelt (George) |
