# GEORGE.md — ToSom i åpen beta: deploy-guide og dagleg oversikt

For George. Alt du skal gjere manuelt står her. Alt anna er allereie i koden.
...
- **Åpen beta er korrleis det alt fungerer.** Éin dør: `/login` — e-post og passord, kontoen lagast automatisk. Ingen invitasjonar, ingen kodar, ingen e-postverifisering. (Dette er BETA-TEST §3.)
- **Denne deploymen inneheld:**
  - Fiksa kvote-tal i admin-panelet: viser no `0 / 5 000` (ikkje hardkoda `10 000`), tersklar 80 %/95 % av det reelle taket.
  - Journey-cron køyrer **timevis** i staden for éin gong per dag, med `JOURNEY_BATCH_SIZE` (300 på Vercel = kapasitet for ~7 200 samtidige reiser i staden for ~100).
  - Ny indikatar i panelet: **«Reiser som venter på fremrykk»** + auto-varsel til e-post når køen heng.
- **Ingen endringar i produktet sjølvt** — same reise, same matcher, same kvote.

---

## 1. Gjer dette, i denne rekkjefølgja
...
- [ ] Kjør migrasjonar mot databasen:
  ```bash
...
> **Kvifor R2?** Utan R2-nøkler fell bilde-lagringa til lokal fil — på Vercel forsvinner dei då ved kvar ny deploy. Reisa er 30 dagar og bilder delast frå dag 15, så R2 er eit krav, ikkje eit val.
...
- [ ] Domenet `tosom.no` må vere verifisert i Resend (DNS)
- [ ] Avsender: **noreplay@tosom.no**

### Steg 6 — E-post-vidarsending (10 minutt)
- [ ] `support@tosom.no` → vidarsend til din private e-post (alle tilbakemeldingar landar her)
...
- [ ] Først **10**, ikkje 50 (DRIFTSPLAN: éi uke observasjon før utviding)
- [ ] Deretter opp mot **50** — taket ditt er mjukt: du berre fortel 50 menneskje
...
|---|---|---|
| `MAINTENANCE_MODE` | (ikkje sett / `false`) | Onns → alt redirectar til `/maintenance`. Det er nødbremsa di. |
| `DEV_LOGIN_ENABLED` | `false` | Må aldri vere på i produksjon. |
| `PAYMENTS_ENABLED` | `false` | `true` gir **fatal feil ved oppstart** (A15) før Vipps er bygd. |
| `BETA_MATCH_EMAIL` | (ikkje sett) | Av i den **første** matcherunden — så vi mår kor mange som oppdager matchen sjølv (I-4-data). Set `true` etter første runde. |

### Valfritt (anbefalt)
| Variabel | Verdi | Kvifor |
|---|---|---|
| `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` | frå Upstash | Distribuert rate-limiting (utan: in-memory per instance — riker for 50 brukere) |
...
## 3. Kill switches (kjører utan ny kode — krev berre redeploy av Vercel)
...
- **Taket: 5 000 gratis reiser** — `PRICING.freeUserCap` i `config/legal.ts`. Samme tal vilkåra lover. Éin kilde.
- **Korleis forbruk:** kvar tester som stiller seg i kø får éin gratisordre. 50 testere ≈ **50 av 5 000** (1 %).
- **Lykkespar gjev 2 tilbake:** når eit par sluttar reisa, slettar systemet ordena — plassane kjem attende i potten.
...
## 5. Kapasitet — kva som held, og når du skal bli uroleg
...
- **«Reiser som venter på fremrykk»** i panelet: `0` = alt fint · `>0` = selvkorrigerer seg innan éin time (bekreft at det blir 0 igjen) · `≥100` = raudt — cronen kjører ikkje (sjekk Vercel → Cron + Logs). Du får òg e-post-varsel.
- **Runde-varigheit:** grønt < 30 s · gult 30–50 s · raudt > 50 s. Med 50 testere skal det vere under 5 s.
- **`rejectReasons`** (rundesloggen): `kjonn` og `alder` er **nye** kategoriar (WP1). Hoge tal = filterbug, ikkje «vanlege avvisningar».
...
2. **Laurdag formiddag:** kom matcherunden? (Siste matcherunde + runde-varigheit + kø = 0?)
3. **support@:** alt svara? (vidarsendt til deg)
4. Noko **raudt**? → §7.

## 7. Kvifor du gjer noko når noko er raudt

1. **Nødbrems** dersom brukarane er påverka: `MAINTENANCE_MODE=true` → redeploy.
...
4. **Aldri** juster tersklar under beta (DI-2) — talet er data til tuning, ikkje ei hending.
...
- Set aldri `PAYMENTS_ENABLED=true` før Vipps er på plass (køyrar ned appen ved oppstart).
- Rør aldri produksjons-DBen direkte.
- Juster ikkje tersklar under beta.
- Opna ikkje for hundrevis av brukere før fase 2 punkt 1 er gjort.
