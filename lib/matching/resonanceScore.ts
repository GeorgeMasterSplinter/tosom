...
/**
 * Bereknar resonans mellom to profiler basert på djup profil-dimensjonar.
 * Returnerer score 0-100 der høgare = djupare resonans.
 */
export function calculateResonance(a: Record<string, unknown>, b: Record<string, unknown>): ResonanceResult {
  let totalScore = 0;
  
  // 1. Verdiar (values) — 25% vekt
  ...
  // 9. Modenheit/trygghet (maturity) — 2% vekt
...
function valueResonance(a: Record<string, unknown>, b: Record<string, unknown>): number {
  ...
  if (!valuesA.length || !valuesB.length) return 50; // neutral ved manglande data
...
/**
 * Relasjonsstil-resonans: kompatibilitet mellom relasjonspreferansar
 */
