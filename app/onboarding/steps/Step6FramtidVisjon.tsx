...
export default function Step6FramtidVisjon({ data, onChange, onBack, onNext }: Props) {
  ...
  return (
    <OnboardingSlide title="Framtid & visjon" subtitle="Hva drømmer du om å bygge?" guidingText="Framtidsønsker viser veien for hva dere kan bygge sammen." slideIndex={7} totalSlides={13}
      ...
      <OnboardingTextField label={`${labels['futureVision']} *`} value={getValue('futureVision', '')} onChange={(v) => onChange('futureVision', v)} placeholder={placeholders['futureVision']} mikroguiding={guiding['futureVision']} maxLength={500} minChars={10} rows={3} multiline testId="ob-future-vision" />
      <div style={{ borderTop: '1px solid rgba(255, 255, 255, 0.06)', marginTop: '20px', marginBottom: '20px' }} />
      <OnboardingTextField label={`${labels['dreamGoal']} *`} value={getValue('dreamGoal', '')} onChange={(v) => onChange('dreamGoal', v)} placeholder={placeholders['dreamGoal']} mikroguiding={guiding['dreamGoal']} maxLength={500} minChars={10} rows={3} multiline testId="ob-dream-goal" />
      <div style={{ borderTop: '1px solid rgba(255, 255, 255, 0.06)', marginTop: '20px', marginBottom: '20px' }} />
      <OnboardingTextField label={`${labels['buildTogether']} *`} value={getValue('buildTogether', '')} onChange={(v) => onChange('buildTogether', v)} placeholder={placeholders['buildTogether']} mikroguiding={guiding['buildTogether']} maxLength={500} minChars={10} rows={3} multiline testId="ob-build-together" />
      <div style={{ borderTop: '1px solid rgba(255, 255, 255, 0.06)', marginTop: '20px', marginBottom: '20px' }} />
