import { execSync } from 'child_process';
import fs from 'fs';

const REPO = 'GeorgeMasterSplinter/tosom';
const SHA = process.argv[2] || '8655a4f';
const LOG = '/tmp/watch-poll.log';

function getToken() {
  try {
    const cred = execSync("printf 'protocol=https\\nhost=github.com\\n\\n' | git credential fill 2>/dev/null", {
      encoding: 'utf8',
    });
    const line = cred.split('\n').find((l) => l.startsWith('password='));
    return line ? line.slice('password='.length).trim() : '';
  } catch {
    return '';
  }
}
const token = getToken();
if (!token) {
  fs.appendFileSync(LOG, 'INGEN TOKEN\n');
  process.exit(1);
}
async function api(path) {
  const res = await fetch('https://api.github.com/repos/' + REPO + path, {
    headers: { 'Authorization': 'token ' + token, 'Accept': 'application/vnd.github+json' },
  });
  return res;
}

let runId = null;
let i = 0;
while (i < 80) {
  i++;
  let line;
  try {
    if (!runId) {
      const j = await (await api('/actions/runs?per_page=15')).json();
      const run = (j.workflow_runs || []).find((r) => r.name === 'ToSom CI' && r.head_sha.startsWith(SHA));
      if (run) runId = run.id;
    }
    if (!runId) {
      line = '[' + new Date().toISOString().slice(11, 19) + '] tick ' + i + ': CI-run for ' + SHA + ' finnes ikke ennå';
    } else {
      const rj = await (await api('/actions/runs/' + runId)).json();
      let e2e = 'n/a';
      try {
        const jobs = await (await api('/actions/runs/' + runId + '/jobs?per_page=40')).json();
        const ej = (jobs.jobs || []).find((x) => x.name.includes('E2E'));
        if (ej) e2e = ej.conclusion || ej.status;
      } catch {}
      line =
        '[' + new Date().toISOString().slice(11, 19) + '] tick ' + i + ': run=' + runId + ' ' + rj.status + '/' + (rj.conclusion || '-') + ' e2e=' + e2e;
      if (rj.status === 'completed') {
        console.log(line);
        fs.appendFileSync(LOG, line + '\nRUN COMPLETED: ' + (rj.conclusion || '-') + '\n');
        console.log('RUN COMPLETED: ' + (rj.conclusion || '-'));
        break;
      }
    }
  } catch (e) {
    line = '[' + new Date().toISOString().slice(11, 19) + '] tick ' + i + ': ERROR ' + e.message;
  }
  console.log(line);
  fs.appendFileSync(LOG, line + '\n');
  await new Promise((r) => setTimeout(r, 90000));
}
fs.appendFileSync(LOG, 'WATCHER EXITED at tick ' + i + '\n');