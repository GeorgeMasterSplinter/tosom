/* ═══════════════════════════════════════════
   ...
   Shimmer + glassmorphism skeletons for all major UI
   ═══════════════════════════════════════════ */
...
export const Skeleton: React.FC<SkeletonProps> = ({ className = "", shimmer = true }) => (
  <div
    className={`skeleton ${shimmer ? "animate-shimmer" : ""} ${className}`}
    role="status"
    aria-busy="true"
    aria-label="Laster innhold"
    style={{ background: "rgba(255,255,255,0.04)", borderRadius: "inherit" }}
  />
);
...
export const MatchCardSkeleton: React.FC = () => (
  <div className="glass-card p-6 animate-shimmer">
    <div className="flex items-start gap-4">
      {/* Avatar */}
      <div
        className="w-16 h-16 rounded-xl bg-white/[0.04]"
        style={{ animation: "shimmer 1.5s ease-in-out infinite" }}
      />
      {/* Content */}
  ...
  <div className="space-y-3 animate-shimmer">
    {Array.from({ length: count }).map((_, i) => (
      <div
        ...
        style={{ animation: `shimmer 1.5s ease-in-out infinite ${i * 100}ms` }}
      >
        {/* Avatar */}
...
export const JourneyCardSkeleton: React.FC = () => (
  <div className="glass-card p-6 animate-shimmer">
    <div className="flex items-center gap-4">
      {/* Icon */}
      <div
        className="w-12 h-12 rounded-lg bg-white/[0.04] flex-shrink-0"
        style={{ animation: "shimmer 1.5s ease-in-out infinite" }}
      />
      {/* Content */}
      <div className="flex-1 flex flex-col gap-2">
  ...
  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 animate-shimmer">
    {Array.from({ length: 4 }).map((_, i) => (
      <div
        ...
        style={{ animation: `shimmer 1.5s ease-in-out infinite ${i * 80}ms` }}
      >
...
export const ResonanceMeterSkeleton: React.FC<{ size?: "sm" | "md" | "lg" }> = ({ size = "md" }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-2 animate-shimmer">
      <div
        className={`${sizeMap[size]} rounded-full bg-white/[0.04]`}
        style={{ animation: "shimmer 1.5s ease-in-out infinite" }}
      />
