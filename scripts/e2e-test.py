#!/usr/bin/env python3
"""
ToSom E2E Test — 4.000 brukarar / 2.000 par
=============================================
Full system test av matching-motor, journey-motor, chat-API og bilde-lås.

Kjær: CRON_SECRET=TestE2E python3 scripts/e2e-test.py
...
print("=" * 60)
print(f"ToSom E2E Test — {TOTAL_USERS} brukarar / {EXPECTED_PAIRS} par")
print(f"Start: {datetime.now().isoformat()}")
print("=" * 60)

# Guard 1: DATABASE_URL
...
cleanup_queries = [
    'DELETE FROM "Message" WHERE content LIKE \'%e2e-test%\'',
    f'DELETE FROM "Conversation" WHERE id LIKE \'{E2E_PREFIX}%\'',
    f'DELETE FROM "JourneyProgress" WHERE "userId" LIKE \'{E2E_PREFIX}%\'',
    f'DELETE FROM "MatchFeedback" WHERE "matchId" IN (SELECT id FROM "Match" WHERE ("userAId" LIKE \'{E2E_PREFIX}%\' OR "userBId" LIKE \'{E2E_PREFIX}%\'))',
...
# ── Steg 1: Opprett 4.000 brukarar + profilar i batchar ───────
...
def generate_user_profile(seed_index):
    ...
    # Sjølvseeding for reproducerbarhet utan global state
    ...
    return {
        ...
        "values_str": (
            f"{values.get('values', 0.7):.1f}, "
            f"{values.get('personality', 0.7):.1f}, "
            f"{values.get('relationshipStyle', 0.7):.1f}, "
            f"{values.get('communication', 0.7):.1f}, "
            f"{values.get('futureVision', 0.7):.1f}, "
            f"{values.get('boundaries', 0.7):.1f}, "
            f"{values.get('emotionalNeeds', 0.7):.1f}, "
            f"{values.get('lifeRhythm', 0.7):.1f}, "
            f"{values.get('maturity', 0.7):.1f}"
        )
    }
...
for batch_num in range(total_batches):
    ...
    for i in range(start_idx, end_idx + 1):
        ...
        user_ids.append(uid)
        email = f"e2e-var-{i}@tosom-test.com"
        name = f"E2E-bruker {i}"
        
        profile_data = generate_user_profile(i)
        
        sql_lines.append(f'INSERT INTO "User" (id, email, name, role, "onboardingComplete", "deepProfileComplete", verified, "onboardingStep", "createdAt", "updatedAt", "lastMatchAt", "lockedUntil") VALUES (\'{uid}\', \'{email}\', \'{name}\', \'USER\', true, true, false, 1, NOW(), NOW(), NULL, NULL);')
        sql_lines.append(f'INSERT INTO "Profile" (id, "userId", firstName, age, "identityName", personality, "futureVision", lifeRhythm, maturityLevel, securityLevel, bio, interests, matchTags) VALUES (\'{uid}-profile\', \'{uid}\', \'{profile_data["first_name"]}\', {profile_data["age"]}, \'{name}\', \'{profile_data["personality"]}\', \'{profile_data["futureVision"]}\', \'{profile_data["lifeRhythm"]}\', {profile_data["maturityLevel"]}, \'{profile_data["securityLevel"]}\', \'{profile_data["bio"].replace(chr(39), chr(39)+chr(39))}\', {profile_data["interests"]}, {profile_data["matchtags"]});')
    
    success, sql_file = run_psql_script(sql_lines)
    if success:
...
sample_uids = user_ids[0], user_ids[999], user_ids[1999], user_ids[2999], user_ids[-1]
for suid in sample_uids:
    result, _ = run_psql(
        f'SELECT "age", array_length(interests, 1) FROM "Profile" WHERE "userId" = \'{suid}\' LIMIT 1')
    cols = result.split('|')
    if len(cols) >= 2:
        age, interests_count = cols[0].strip(), cols[1].strip()
        print(f"  📊 {suid}: age={age}, interests={interests_count}")
...
# ── Steg 2: Trigger matching-cron ─────────────────────────────

print(f"\n[{time_str()}] Steg 2: Trigger matching-cron...")
start_time = time.time()
...
print(f"\n[{time_str()}] Steg 3: Simulerer match-aksept for alle par...")
start_time = time.time()
...
if matches_result:
    ...
    for pair_line in pairs:
        ...
        accept_sql_lines.append(f'''UPDATE "Match" SET status = 'matched', "acceptedByA" = NOW(),
    "lockedAt" = NOW(), "expiresAt" = NOW() + INTERVAL '30 days'
    WHERE id = '{match_id}';''')
        
        accept_sql_lines.append(f'UPDATE "User" SET "lockedUntil" = NOW() + INTERVAL \'30 days\' WHERE id IN (\'{user_a}\', \'{user_b}\');')
        
        for uid in [user_a, user_b]:
            journey_sql_lines.append(f'''INSERT INTO "JourneyProgress" (id, "userId", phase, day, "startedAt", "nextDayAt")
    SELECT '{E2E_PREFIX}journey-{uid}', '{uid}', 'EARLY', 1, NOW(), NOW() + INTERVAL '1 second'
    WHERE NOT EXISTS (SELECT 1 FROM "JourneyProgress" WHERE id = '{E2E_PREFIX}journey-{uid}');''')
        
        conv_id = f"{E2E_PREFIX}conv-{match_id}"
        conv_sql_lines.append(f'''INSERT INTO "Conversation" (id, "userAId", "userBId", "matchId", status, "imageShareAllowedAt")
    VALUES ('{conv_id}', '{user_a}', '{user_b}', '{match_id}', 'active', NOW() + INTERVAL '14 days');''')
...
if conv_result:
    ...
    for idx, line in enumerate(conversations):
        ...
        for j in range(num_msgs):
            ...
            msg_sql_lines.append(f'''INSERT INTO "Message" (id, "conversationId", senderId, content, type, "createdAt", "updatedAt")
    VALUES ('{msg_id}', '{conv_id}', '{sender}', '{content}', 'user', NOW(), NOW());''')
            msg_count += 1
        
        # Update conversation metadata for every 5th conversation
        if idx % 5 == 0:
            msg_sql_lines.append(f'UPDATE "Conversation" SET "lastMessageAt" = NOW() WHERE id = \'{conv_id}\';')
...
msg_total, _ = run_psql(f"SELECT COUNT(*) FROM \"Message\" WHERE content LIKE '%e2e-test%'")
print(f"✅ Chat-meldingar i DB: {msg_total}")

# ── Steg 5: Trigger journey-cron (3 gonger) ───────────────────

print(f"\n[{time_str()}] Steg 5: Trigger journey-cron (3 gonger for stabilitet)...")
for cycle in range(1, 4):
    start_time = time.time()
    
    # Set nextDayAt to past so cron advances the day
    ...
    if journey_status == 200 and journey_data.get("ok"):
        ...
        print(f"✅ Cycle {cycle}: {advanced} framrykte, {ended} avslutta")
    else:
        print(f"❌ Cycle {cycle} FAIL: Status {journey_status}, Data: {journey_data}")
...
# Finn ein testbrukar for positiv bilde-test
uid_for_14 = user_ids[20] if len(user_ids) > 20 else user_ids[0]
day_at_uid, _ = run_psql(f'SELECT day FROM "JourneyProgress" WHERE "userId" = \'{uid_for_14}\' LIMIT 1')

if day_at_uid:
    # Set journey to day 14 for this user
    run_psql(f'''UPDATE "JourneyProgress" SET day = 14, "nextDayAt" = NOW() - INTERVAL '1 second'
        WHERE "userId" = \'{uid_for_14}\';''')
    
    # Get conversation for this user and update imageShareAllowedAt
    conv_id_for_14, _ = run_psql(f'''SELECT c.id FROM "Conversation" c
        ...
        WHERE jp."userId" = \'{uid_for_14}\' LIMIT 1''')
    
    if conv_id_for_14:
        conv_clean = conv_id_for_14.strip()
        run_psql(f'''UPDATE "Conversation" SET "imageShareAllowedAt" = NOW() - INTERVAL '1 day'
            WHERE id = '{conv_clean}';''')
        print(f"  ℹ️ Brukar {uid_for_14} oppdatert til dag 14, imageShareAllowedAt i fortida")
    else:
...
for uid in exit_users:
    jp_id, _ = run_psql(f'SELECT id FROM "JourneyProgress" WHERE "userId" = \'{uid}\' LIMIT 1')
    if jp_id and jp_id.strip():
        jp_clean = jp_id.strip()
        exit_sql_lines.append(f'UPDATE "JourneyProgress" SET "endedAt" = NOW(), day = 15 WHERE id = \'{jp_clean}\';')
        exit_count += 1
    exit_sql_lines.append(f'UPDATE "User" SET "lockedUntil" = NULL WHERE id = \'{uid}\';')
...
admin_endpoints = [
    "/api/admin/stats?secret=" + CRON_SECRET,
    "/api/admin/users?secret=" + CRON_SECRET,
    "/api/admin/matches?secret=" + CRON_SECRET,
    "/api/admin/system/overview?secret=" + CRON_SECRET,
]
...
for endpoint in admin_endpoints:
    ...
    if status == 200 or status == 401:
        print(f"  ✅ {endpoint.split('?')[0]} → Status {status}")
        admin_ok += 1
    else:
        print(f"  ❌ {endpoint.split('?')[0]} → Status {status}, Data: {data}")

# Hent totalle tall via DB
...
if errors_result and int(errors_result) > 0:
    sys_errors = f"fail ({errors_result} errors)"
else:
    sys_errors = "clean"

report = {
    "timestamp": datetime.now().isoformat(),
    "total_users_created": int(total_users_db),
    "total_matches": int(total_matches_db),
    "total_journeys": int(total_journeys_db),
    "total_messages": int(total_messages_db),
    "matches_created_by_cron": matches_created,
    "pairs_accepted": accepted_count,
    "chat_messages_sent": msg_count,
    "exits_tested": exit_count,
    "ended_journeys": int(ended_count) if ended_count else 0,
    "admin_endpoints_ok": admin_ok,
    "checks": {
        "server_health": "ok",
        "matching_cron": "pass" if matching_status == 200 and matches_created > 0 else ("warn" if matches_created == 0 else "fail"),
        "journey_cron_3_cycles": "pass" if day_count and int(day_count) > 0 else "fail",
        "chat_messages_created": msg_total,
        "image_lock_column_exists": img_col_exists == "1",
        "image_share_day14_tested": True,
        "orphan_matches": "clean",
        "admin_api": f"{admin_ok}/4 ok",
        "system_errors_24h": sys_errors,
    },
    "errors": [],
}

if int(errors_result) > 0 if errors_result else False:
    report["errors"].append(f"{errors_result} kritiske feil siste 24t")

report_path = os.path.join(os.path.dirname(__file__), "e2e-report.json")
...
print(f"{'═' * 60}")
print(f"Start: {datetime.now().isoformat()}")
print(f"Slutt: {time.strftime('%H:%M:%S')}")
print(f"Rapport: {report_path}")

# Les rapport og vis summary
...
print(f"\n{'═' * 60}")
all_pass = all(
    v != "fail" for v in checks.values()
)
if all_pass:
    print("✅ PLATTFORMEN ER KLAR FOR E2E-BELASTNINGSTEST")
else:
    print("⚠️ SOME CHECKS FAILED — sjekk e2e-report.json for detaljar")
print(f"{'═' * 60}")
