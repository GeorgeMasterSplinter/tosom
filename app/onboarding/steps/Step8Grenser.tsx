...
export default function Step8Grenser({ data, onChange, onBack, onNext }: Props) {
  ...
  return (
    <OnboardingSlide title="Grenser & behov" subtitle="Hvordan ser du på grenser i en relasjon — og hva trenger du at partneren din forstår?" guidingText="Grenser beskytter deg selv — og den du elsker." slideIndex={9} totalSlides={13}>
      ...
      <div style={{ borderTop: '2px solid rgba(212, 175, 55, 0.2)', marginTop: '20px', marginBottom: '20px' }} />
      <OnboardingTextField label="Hva er dine viktigste avgrensninger? *" value={getValue('limitations', '')} onChange={(v) => onChange('limitations', v)} placeholder="Skriv f.eks. Jeg trenger aldri å bli møtt med høye stemmer" mikroguiding="Skriv f.eks. Jeg trenger aldri å bli møtt med høye stemmer" maxLength={300} minChars={10} rows={3} multiline />
      <div style={{ borderTop: '2px solid rgba(212, 175, 55, 0.2)', marginTop: '20px', marginBottom: '20px' }} />
      <OnboardingTextField label="Hva må partneren din forstå om deg? *" value={getValue('partnerMustUnderstand', '')} onChange={(v) => onChange('partnerMustUnderstand', v)} placeholder="Skriv f.eks. At jeg trenger tid til å bearbeide følelser alene" mikroguiding="Skriv f.eks. At jeg trenger tid til å bearbeide følelser alene" maxLength={300} minChars={10} rows={3} multiline />
