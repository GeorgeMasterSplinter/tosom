...
export default function Step5LivsstilVerdier({ data, onChange, onBack, onNext, step, goToStep }: Props) {
  ...
  return (
    <OnboardingSlide title="Livsstil & verdier" subtitle="Hva prioriterer du i hverdagen?" guidingText="Livsstilssvarene dine hjelper oss å finne noen som trives i hverdag sammen med deg." slideIndex={5} totalSlides={13}
      ...
      <OnboardingSelectGrid label="Hva er mindre viktig for deg? *" mikroguiding="Velg det som betyr minst for deg nå" options={[{ value: 'materiell', label: 'Materielle ting', icon: '💰' }, { value: 'status', label: 'Status og anerkjennelse', icon: '🏆' }, { value: 'sosial-media', label: 'Sosialt mediabruk', icon: '📱' }, { value: 'sport', label: 'Sport og konkurranse', icon: '⚽' }, { value: 'underholdning', label: 'Underholdning og kos', icon: '🎬' }]} selectedValue={getValue('lowPriority', '')} onChange={(v) => onChange('lowPriority', v)} />
      <div style={{ borderTop: '1px solid rgba(255, 255, 255, 0.06)', marginTop: '20px', marginBottom: '20px' }} />
      ...
      <div style={{ borderTop: '1px solid rgba(255, 255, 255, 0.06)', marginTop: '20px', marginBottom: '20px' }} />
      <OnboardingSelectGrid label="Hvordan ønsker du å leve? *" mikroguiding="Dette er hva du drømmer om" options={[{ value: 'roleg', label: 'Rolig og forutsigbart', icon: '🌿' }, { value: 'eventyr', label: 'Eventyr og endring', icon: '🧭' }, { value: 'balansert', label: 'Balansen mellom ro og aktivitet', icon: '⚖️' }, { value: 'skapende', label: 'Skapende og kunstnerisk', icon: '🎨' }]} selectedValue={getValue('desiredLifestyle', '')} onChange={(v) => onChange('desiredLifestyle', v)} />
      <div style={{ borderTop: '1px solid rgba(255, 255, 255, 0.06)', marginTop: '20px', marginBottom: '20px' }} />
